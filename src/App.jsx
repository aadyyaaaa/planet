...
function App() {
  ...
  const reset = () => { setConfig(defaults); setStats(calculateStats(defaults)); setProfile(null); setEvent(null); setEventBoosts({}); setDestroyed(false); setConfirmDestroy(false); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  const randomize = () => { const next = randomConfig(); setConfig(next); setProfile(null); setEvent(null); setEventBoosts({}); setDestroyed(false); setToast({ type: 'success', message: 'A new world has entered orbit.' }) }
